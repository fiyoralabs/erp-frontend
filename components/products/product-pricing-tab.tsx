...
export function ProductPricingTab({ productId, companyId, hasVariants }: {
  ...
  React.useEffect(() => {
    ...
    setRows(identities.map((identity) => {
      ...
      return {
        ...
        costPrice: price ? String(price.costPrice) : "",
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const invalid = rows.find((row) => Number(row.costPrice) <= 0 || Number(row.sellingPrice) <= 0);
      if (invalid) throw new Error(`Enter valid cost and selling prices for ${invalid.label}`);
  ...
  function updateRow(index: number, field: "costPrice" | "sellingPrice" | "mrp", value: string) {
  ...
  function copyFirstToAll() {
    ...
    setRows((current) => current.map((row, index) => index === 0 ? row : {
      ...row, costPrice: first.costPrice, sellingPrice: first.sellingPrice, mrp: first.mrp,
    }));
  }
    ...
    {priceListId != null && <Card>
      ...
      <CardContent className="flex flex-col gap-4">
        {pricesQuery.isLoading || variantsQuery.isLoading ? <p className="text-sm text-muted-foreground">Loading price worksheet...</p> : rows.length === 0 ? <div className="rounded-lg border border-dashed p-6 text-center"><p className="font-medium">No active variants to price</p><p className="text-sm text-muted-foreground">Generate variants in the Variants tab first.</p></div> : <>
          <div className="hidden grid-cols-[minmax(220px,1fr)_140px_140px_140px] gap-3 border-b pb-2 text-xs font-medium text-muted-foreground md:grid"><span>Sellable SKU</span><span>Cost price</span><span>Selling price</span><span>MRP</span></div>
          {rows.map((row, index) => <div key={row.variantId ?? "product"} className="grid grid-cols-1 gap-3 rounded-lg border p-3 md:grid-cols-[minmax(220px,1fr)_140px_140px_140px] md:items-center md:border-0 md:border-b md:px-0">
