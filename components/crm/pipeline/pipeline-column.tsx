...
export function PipelineColumn({ stage, opportunities }: { stage: PipelineStage; opportunities: Opportunity[] }) {
  ...
  return (
    <div className="flex w-72 shrink-0 flex-col gap-2">
      <div className="flex items-center justify-between rounded-t-md px-2 py-1.5" style={{ borderTop: `3px solid ${stage.color ?? "#94a3b8"}` }}>
        <div>
          <p className="text-sm font-semibold">{stage.name}</p>
          <p className="text-xs text-muted-foreground">{opportunities.length} deals · {formatCurrency(totalValue)}</p>
