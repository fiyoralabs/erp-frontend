...
export function SidebarNav({ permissions, onNavigate }: { permissions: readonly string[]; onNavigate?: () => void }) {
  ...
  return (
    <nav className="flex flex-col gap-1 p-3">
      {navItems.filter((item) => isVisible(item, permissions)).map((item) => {
...
function NavGroup({
  ...
  const visibleChildren = (item.items ?? []).filter((child) => isVisible(child, permissions));
      ...
      {open && (
        <div className="ml-4 flex flex-col gap-1 border-l pl-3 pt-1">
          {visibleChildren.map((child) => (
            <NavLink key={child.href} item={child} pathname={pathname} onNavigate={onNavigate} exact />
          ))}
        </div>
      )}
    </div>
