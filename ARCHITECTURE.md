...
- **Form buttons need an explicit `type="submit"`.** Base UI's `Button` defaults to `type="button"`, so a visually correct button inside a `<form>` otherwise does not fire `onSubmit` and produces no network request. This caused the forgot-password "Send code" button to appear inert.
